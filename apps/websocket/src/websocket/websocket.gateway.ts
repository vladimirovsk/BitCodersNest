  ...
  implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect
{
  ...
  async eventSendMessage() {
    if (this.startSendMessageRate) {
      ...
      const currentTokenInfo: IWebsocketMessage | null | undefined =
        await this.websocketService.fetchDataMessageTokens();
  ...
  async handleSubscribe(@MessageBody() data: IWebsocketMessage) {
    ...
    const currentTokenInfo = await this.websocketService.fetchDataMessageTokens(
      true,
    );
  ...
  afterInit(server: Server) {
    this.logger.debug(`INIT SOCKET SERVER ${Server.name}`);
  }
